...
In order to understant how Narrowband Internet-of-thing (NB-IOT) works, I first need to understand how LTE works, because they share many designs in the physical architecture.

Then I learn from a Youtube channel whose name is exactly "[LTE](https://www.youtube.com/channel/UCf5srFJ-JofnE8r-bn1o1VA)". They posts about 12 videos regarding the fundamental of LTE (can be found [here](https://www.youtube.com/playlist?list%3DPLE6yE0jB6BTOY6Z1DKEkQ8yZ8fFPUiCD8)). This is my note for it. However, I started taking note from the 5th video. So I don't have anything to post for the previous series.


## LTE logical channel

Responsibility of LTE logical channel
...
In data link layer (logical channel), Radio link control layer is responsible for RLC segmaentation / concatenation.
...
-   C1 - PCCH: paging control. Determine if the mobile is in idle state.
-   C2 - BCCH: broadcast message :
    -   Downlink bandwidth
    -   Antenna configuration
    -   Reference signal power
-   C3 - CCCH: (C for "connection") mobile device's attempt to latch to the network, establishing connection after powered on.
...
-   U2 - MTCH: boardcast data to multiple users (in sport or event scenarios etc.)
...
For uplink, there is only one transport channel, which is UL-SCH. The data from CCCH, DCCH, DTCH will be forwarded to UL-SCH.
...
3.  HARQ retransmission control.
    -   At the first place, Tx MAC layer sends HARQ to Rx MAC. If Rx can not decode or it is corrupted, Rx will respond an NACK. Then Tx MAC is responsible for retransmission.
...
Then there are 14=5+4+3+2 bits in the channel, and 6 bits of empty room.
...
Paging signals are retransmitted on fixed intervals, based on discontinuous reception or DRX inactivity timer. It is not depend on HARQ response.

1.  Allignment of uplink timing and power headroom reporting

The most important function of eNB is to allocate resources dynamically

The mobile device sent channel quality indicator (CQI) aka channel state information messages, to the cell. In response, the cell sends modulation scheme to the mobile device. When the cell sending user data, it decide to use whether QPSK or 16-64QAM depending on the channel quality. However, signalling traffic is always transferred by QPSK. QPSK is the most robust modulation scheme.
...
-   It provides the following function:
    ...
    -   Information about the SIB need by user to transmit uplink data. It is mandatory for the user to decode to successfully transmit and receive data.
...
-   It indicates the number of ODFM symbols used for the PDCCH, whether 1, 2, or 3.
-   Information within this channel is essential to decode PDCCH.


## Physical signal

Physical signal is different from physical channels. It is before the mixing RF part.

Work flow of physical singal before the electromegnatic wave is transmitted in the air:
